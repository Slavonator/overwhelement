...
*TODO: рисунок изображающий несколько вьюпортов*
...
*TODO: дописать `examples/readme_images/multiple_viewports.rs`*

Но внутри одной этой плоскости, порядок перечисления вьюпортов не даёт им приоритета друг над другом: имеет значение только глубина объектов, которые включает в себя вьюпорт. В итоге, если на снимке есть фигура, которая целиком или частично перекрывает то, что позади (то есть её глубина `depth` меньше, чем у фигуры позади, и она оказывается ближе к наблюдателю), то она перезаписывает прошлые видимые дискреты своими, более близкими к наблюдателю, значениями, создавая коллаж из единой плоскости.
