...
Overwhelement представляет ошеломляюще гибкую настройку входных данных, и она может показаться неочевидной на первый взгляд. Для простоты посмотрим на [`Plane`](docs/API#Plane). Он содержит в себе индексы на список [`Viewport`](docs/API#Viewport) в общем конейнере - сцене. [`Viewport`](docs/API#Viewport) определяет то, какой участок плоскости будет виден, и дискретизирован.
