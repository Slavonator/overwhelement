...
Overwhelement - преобразователь двухмерных плоскостей в дискретные информативные структуры данных. Он принимает в себя стопку плоскостей, которые содержат линии и треугольники, вершины которых заданы в экранных координатах, а затем возвращает результат наложения этих плоскостей друг на друга в виде буфера элементов. За один проход дискретизации, из буфера элементов можно получить фреймбуфер, карту высот, карту освещения, или иерархию слоёв. Буфер элементов можно использовать для вывода в графическое окно, терминал, ASCII изображение, LED-панель, E-Ink дисплей, преобразовать в tensor для ML, на матрицу из ламп, или куда-угодно ещё, на что мне не хватило фантазии. Код полностью сгенерирован, никакого человеческого интеллекта в создании задействовано не было.

---

## [API](docs/API.md)

API представлен в API.md репозитория. Конкретные структуры и функции лучше смотреть там, так как в самом коде комментарии не информативны, хаотичны или отсутствуют. Для удобного взаимодействия лучше смотреть туда, там есть подробное описание всех публичных структур, функций и методов.
...
> Диаграмма сделана с использованием Overwhelement, её код представлен в [`examples/pipeline_diagram.rs`](examples/readme_images/pipeline_diagram.rs)
...
Overwhelement представляет ошеломляюще гибкую настройку входных данных, и она может показаться неочевидной на первый взгляд. Для простоты посмотрим на [`Plane`](docs/API.md#Plane). Он содержит в себе индексы на список [`Viewport`](docs/API.md#Viewport) в общем конейнере - сцене. [`Viewport`](docs/API.md#Viewport) определяет то, какой участок плоскости будет виден, и дискретизирован.

![Viewport](docs/viewport.png)
> Рисунок сделан с использованием Overwhelement, его код представлен в [`examples/plane_viewport.rs`](examples/readme_images/plane_viewport.rs)

По этому рисунку видно, как вьюпорт занимает лишь определенный участок плоскости, чтобы задать рамки для дискретизации. Всё, что внутри вьюпорта будет дискретизироваться, а остальное - отсекаться. Это вырезка, или снимок плоскости, которая выставляется перед наблюдателем. В Overwhelement [`Viewport`](docs/API.md#Viewport) можно свободно двигать, масштабировать и поворачивать, чтобы захватить только ту часть [`Plane`](docs/API.md#Plane), которую нужно дискретизировать. У одного [`Plane`](docs/API.md#Plane) может быть множество вьюпортов. При дискретизации, в своей внутренней реализации, Overwhelement проходит по каждому из вьюпортов, и по очереди, последовательно выставляет их перед наблюдателем, собирая коллаж из различных фрагментов одной и той же плоскости но разных её "снимков". 

![Перекрытия viewport](docs/multiple_viewports.png)
> Рисунок сделан с использованием Overwhelement, его код представлен в [`examples/readme_images/multiple_viewports.rs`](examples/readme_images/multiple_viewports.rs)

Но внутри одной этой плоскости, порядок перечисления вьюпортов не даёт им приоритета друг над другом: имеет значение только глубина объектов, которые включает в себя вьюпорт. На рисунке выше, в левой части изображена плоскость, и два прямоугольника - каждому соответсвует свой вьюпорт. В правой части результат их наложения друг на друга. Из рисунка видно, что вьюпорты не перезаписывают, а дополняют друга. В итоге, если на снимке есть фигура, которая целиком или частично перекрывает то, что позади (то есть её глубина `depth` меньше, чем у фигуры позади, и она оказывается ближе к наблюдателю), то она перезаписывает прошлые видимые дискреты своими, более близкими к наблюдателю значениями, создавая коллаж из единой плоскости.
